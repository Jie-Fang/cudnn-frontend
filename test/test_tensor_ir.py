...
class test_tensor_ir:
    ...
    def determine_tensor_ir_inout_tensor_type(self, node, dtype = {}):
        ...
        if dtype is self.determine_tensor_ir_inout_tensor_type.__defaults__[0] : dtype = self.determine_tensor_ir_dtype(test_tensor.data_type)
        else : dtype = self.determine_tensor_ir_dtype(dtype)
        ...
        stride = []
        idx = 0
        for s, d in zip(test_tensor.cudnn_tensor.get_stride(), test_tensor.cudnn_tensor.get_dim()):
            if idx > 0 and d == 1:
                shape.append(1)
                stride.append(0)
            else:
                ...
                shape.append(-1)
            idx += 1
        ty = nv_tensor_ir.TensorType.get(shape=test_tensor.cudnn_tensor.get_dim() , stride=test_tensor.cudnn_tensor.get_stride(), datatype = dtype)
    ...
    def construct_tensor_ir_tensor_type(self, node, data_type):
        assert(len(node.output) == 1)
        test_tensor = node.output[0]
        dtype = self.determine_tensor_ir_dtype(data_type)
        shape = []
        stride = []
        for s in test_tensor.cudnn_tensor.get_stride():
            if s != 1:
                stride.append(-1)
            else:
                stride.append(1)
    ...
    def tensorir_compare_to_reference(self, atol=1e-2, rtol=1e-2):
        ...
        for Y_expected, Y_actual in zip(ref_outputs, self.test_graph.getOutputs()):
            ...
            if Y_expected is None:
                continue
            print("actual_output[", output_idx, "]", Y_actual)
            print("ref_output[", output_idx, "]", Y_expected)
            ...
            try:
                ...
                print("output[",output_idx,"] MISMATCH!", error)
            else:
                print("output[",output_idx,"] PASSED!")
    ...
    def run_tensor_ir_module(self, module,
        compile_option = nv_tensor_ir.TensorConversionOptions([128, 128, 64], [128, 128, 16], [1, 1, 1], 1)):
        kernel_name = 'graph'
        graph_analysis = nv_tensor_ir.GraphAnalysis(module)
        a_partition_idx = 0
        b_partition_idx = 0
        for i in range(len(self.test_graph.entrance_nodes)):
            if nv_tensor_ir.GraphPartitionType.GRAPH_PARTITION_MAINLOOP_A == graph_analysis.get_graph_operand_partition(module, i):
                a_partition_idx = i
            if nv_tensor_ir.GraphPartitionType.GRAPH_PARTITION_MAINLOOP_B == graph_analysis.get_graph_operand_partition(module, i):
                b_partition_idx = i
        a_tensor_dim = self.test_graph.entrance_nodes[a_partition_idx].output[0].cudnn_tensor.get_dim()
        b_tensor_dim = self.test_graph.entrance_nodes[b_partition_idx].output[0].cudnn_tensor.get_dim()
        B = a_tensor_dim[0]
        M = a_tensor_dim[1]
        N = b_tensor_dim[2]
        K = a_tensor_dim[2]
        ...
        for node in self.test_graph.entrance_nodes:
            ...
            if not node.output[0].is_by_value:
                ...
                tensor_operand = nv_tensor_ir.TensorOperandDescriptor(
                    nv_tensor_ir.TensorDescriptor(gpu_mem.data_ptr(), nv_tensor_ir.LayoutDescriptor(torch_mem.stride()))
                )
                tensor_desc.append(tensor_operand)
            else:
                tensor_operand = nv_tensor_ir.TensorOperandDescriptor(
                    nv_tensor_ir.ScalarDescriptor(self.determine_cask_dtype(node.output[0].data_type), torch_mem[0].data_ptr())
                )
                tensor_desc.append(tensor_operand)
        ...
        for output in outputs:
            ...
            tensor_operand = nv_tensor_ir.TensorOperandDescriptor(
                nv_tensor_ir.TensorDescriptor(torch_gpu.data_ptr(), nv_tensor_ir.LayoutDescriptor(output.stride()))
            )
            tensor_desc.append(tensor_operand)
    ...
    def build_tensor_ir_module(self):
        input_tensors = self.test_graph.entrance_nodes
        ...
        with ir.Context() as ctx, ir.Location.unknown() as loc:
            ...
            input_types = list(map(lambda x: next(iter(self.determine_tensor_ir_inout_tensor_type(x))), input_tensors))
            output_types = list(map(lambda x: next(iter(self.determine_tensor_ir_inout_tensor_type(x))), output_tensors))
            ...
            graph = nv_tensor_ir.graph(function_name, function_type=function_type, ip=ir.InsertionPoint(module.body))
            ...
            with ir.InsertionPoint(graph.regions[0].blocks[0]) as ip:
                for node in output_tensors: # This should be built from output nodes no?
                    ...
                    self.build_tensor_ir_recursive(node, node_map, ip) # This is passed the insertion point, which provides a mutable reference to the module. # Consider passing other things, as order in graph module is said to not matter.

                # Generate return op
                result = []
                for node in output_tensors:
                    if 'compute_data_type'in node.kwargs and node.output[0]._data_type !=  node.kwargs['compute_data_type']:
                        node_map[node] = nv_tensor_ir.convert(next(iter(self.determine_tensor_ir_inout_tensor_type(node, node.output[0]._data_type))), node_map[node])
    ...
    def build_binary_operation(self, op_name, node, children, out_type, node_map):
        lsh, rsh = self.convert_and_splat_for_binary_pointwise(children[0], children[1], out_type)
        match op_name:
            case "add":
            ...
            case _:
                print("Unimplemented binary Operation in Lowering to Tensor IR: ", op_name)
    ...
    def build_unary_operation(self, op_name, node, children, out_type, node_map):
        ...
        match op_name:
            ...
            case _:
                print("Unimplemented unary Operation in Lowering to Tensor IR: ", op_name)
    ...
    def build_tensor_ir_recursive(self, node, node_map, ip):
        ...
        with ip as InsertionPoint:
            ...
            if isinstance(node, tg.operation):
                ...
                out_type, out_shape, out_stride, _ = self.determine_tensor_ir_inout_tensor_type(node, node.kwargs['compute_data_type'] if 'compute_data_type' in node.kwargs else node.output[0].data_type)
                match node.cudnn_op.__name__: # FIXME(@xrouth): Try to match on something less fragile than "__name__"
                    case "reduction":
                        accumulator_type = ir.TypeAttr.get(self.determine_tensor_ir_dtype(self.test_graph.compute_data_type))
                        ...
                        print("out_type:", out_type, "out_shape:", out_shape, " out_stride:", out_stride)
                        reduction_dim = 0
                        for s in out_stride:
                            if s == 0:
                                break
                        ...
                        mlir_value = nv_tensor_ir.reduce(out_type, children[0], reduction_dimensions, reduction_mode)
                        node_map[node] = mlir_value
                    case "matmul":
                        compute_data_type = node.kwargs['compute_data_type']
                        node_output = node.output[0]
                        compute_type, _, _, _ = self.determine_tensor_ir_inout_tensor_type(node, compute_data_type)
                        matmul_value = nv_tensor_ir.matmul(compute_type, children[0], children[1])
                        node_map[node] = matmul_value
                    # POINTWISE:
                    case "bias" | "add" | "sub" | "mul" | "max" | "min" | "div" | "mod" | "add_square" | "logical_or" | "logical_and" | "logical_not" | "relu_backward":
                        self.build_binary_operation(node.cudnn_op.__name__, node, children, out_type, node_map)
                    case "tanh" | "abs" | "ceil" | "floor" | "cos" | "sin" | "tan" | "exp" | "log" | "neg" | "rsqrt" | "ert" | "reciprocal" | "relu" | "sigmoid" | "elu" | "gelu" | "gelu_approx_tanh":
                        self.build_unary_operation(node.cudnn_op.__name__, node, children, out_type, node_map)
                    case "cmp_lt":
                        comparator = None
                        node_map[node] = nv_tensor_ir.cmp(out_type, comparator, children[0], children[1])
                    case "cmp_ge":
                        comparator = None
                        node_map[node] = nv_tensor_ir.cmp(out_type, comparator, children[0], children[1])
                    case "cmp_gt":
                    ...
                    case "pow":
                        ...
                        node_map[node] = nv_tensor_ir.pow(out_type, children[0], children[1])
                    case "identity":
                        node_map[node] = node
                    case "conv_wgrad":
                        dy = children[0]
                        w = children[1]
                        accumulator_type_attr = ir.TypeAttr.get(self.determine_tensor_ir_dtype(self.test_graph.compute_data_type))

                        out_dims = node.output[0].cudnn_tensor.get_dim()

                        pre_padding = node.kwargs["padding"]
                        post_padding = node.kwargs["padding"] #  [] # FIXME(@xrouth): what should this be?
                        stride = node.kwargs["stride"]
                        dilation = node.kwargs["dilation"]

                        node_map[node] = nv_tensor_ir.conv_dgrad(out_type, dy, w, accumulator_type_attr, pre_padding, post_padding, stride, dilation)
                    case "conv_dgrad":
                        dy = children[0]
                        w = children[1]
                        accumulator_type_attr = ir.TypeAttr.get(self.determine_tensor_ir_dtype(self.test_graph.compute_data_type))
                    ...
                    case "conv_fprop":
                        ...
                        node_map[node] = nv_tensor_ir.conv_fprop(out_type, x, w, accumulator_type_attr, pre_padding, post_padding, stride, dilation)
                    case _:
                        print("Unimplemented Operation in Lowering to Tensor IR: ", node.cudnn_op.__name__)
            else:
                print("not an op")
                # Nothing