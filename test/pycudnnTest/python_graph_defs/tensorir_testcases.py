...
def test_gemm(jparams, testgraph):
    ...
    gemm_output = testgraph.matmul(name="mb_matmul", A=tensor_a, B=tensor_b, compute_data_type=DataType.HALF)
...
def test_gemm_relu(jparams, testgraph):
    ...
    gemm_output = testgraph.matmul(name="mb_matmul", A=image, B=weight, compute_data_type=DataType.FLOAT)
