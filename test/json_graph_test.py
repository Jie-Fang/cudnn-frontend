from utils import (
    getFwdConvOutputDim,
    computeStrideNdTransposedPacked,
    reportCurrentTime,
    ImplementationError,
    create_nhwc_strides,
)
...
def replace_single_param(json_test_def, abstract_params):
    ...
    catch_all = {
        "formatIn": FORMAT_ALL,
        "filtFormat": FORMAT_ALL,
        "formatOut": FORMAT_ALL,
        "Pin": IO_DATA_TYPE,
        "Pout": IO_DATA_TYPE,
    }
...
def run_test_from_legacy_args(parent_args, unparsed_graphRunner_args):
    ...
    l_parser.add_argument(
        ...
        dest="json_fname",
        action="store",
        default=os.path.join(SCRIPT_DIR, "json_graph_defs", "fusionGraphTests.json"),
    )
    l_parser.add_argument(
        "-T",
        dest="timing_loop",
        ...
        type=int,
        default=0,
        help="positive value: the number of times to run the kernel, no refcheck; 0: run kernel once, run refcheck",
    )
    l_parser.add_argument(
        "-p",
        dest="pseudo_random_generator",
        action="store",
        choices=["r"],
        help='Only normal distribution ("r") supported',
    )
    ...
    l_parser.add_argument(
        ...
        dest="key_values",
        default=[],
        help="kv values to be specified as [-kv=<key> <value>]+ e.g., -kv=layout NCHW.",
    )
    ...
    l_parser.add_argument(
        ...
        dest="convMode",
        action="store_const",
        const="CUDNN_CROSS_CORRELATION",
        default="CUDNN_CONVOLUTION",
    )
    ...
    l_parser.add_argument(
        ...
        type=int,
        choices=[1],
        help="Dummy argument to support default convolution with alpha=1",
    )
    l_parser.add_argument(
        "-B",
        type=int,
    ...
    l_parser.add_argument(
        "-P",
        help="Data type for both I/O in case Pin or Pout is not specified",
        choices=kDATA_TYPES,
    )
    ...
    try:
        ...
        testGraph = setup_test_graph_from_json(
            concrete_test_dict, legacy_args.backendEngine
        )

        if legacy_args.tensor_ir:
            run_tensor_ir_test_from_json_definition(legacy_args, testGraph, concrete_test_dict)
...
def get_tensorir_compilation_config(legacy_args, concrete_test_dict):
    if hasattr(legacy_args, 'tile_size'):
        concrete_test_dict['tile_size'] = list(map(int, legacy_args.tile_size.split(",")))
    if hasattr(legacy_args, 'cluster_shape'):
        concrete_test_dict['cluster_shape'] = list(map(int, legacy_args.cluster_shape.split(",")))
    if hasattr(legacy_args, 'mma_shape'):
        concrete_test_dict['mma_shape'] = list(map(int, legacy_args.mma_shape.split(",")))
        ...
        for output in test_graph_op.output:
            for tensor_name, output_info in TGTensors.items():
                if output.name == output_info.name:
                    for jtensor in jtensor_dict:
                        if tensor_name == jtensor['name'] and ('isVirtual' not in jtensor or jtensor['isVirtual'] == False):
...
def run_tensor_ir_test_from_json_definition(legacy_args, testGraph, json_dict):
    ...
    options = nv_tensor_ir.TensorConversionOptions(tile_size, mma_shape, cluster_shape, cta_count)
