...
def _allocate_inputs(cfg, next_n: int):
    ...
    assert n_rows % next_n == 0, (
        f"n_rows={n_rows} must be divisible by next_n={next_n}"
    )
    ...
    seq_lens = torch.randint(
        lo, s_kv + 1, (batch_size,), dtype=torch.int32, device=device,
    )
...
def test_DSA_indexer_top_k_compile_execute(
    dtype, acc_dtype, top_k, next_n, return_val, request,
):
    try:
    ...
    if not cfg["skip_ref"]:
        check_ref_indexer_top_k(
            input_values, seq_lens, top_k, next_n, indices, values, return_val,
        )
...
def test_DSA_indexer_top_k_wrapper(
    ...
    try:
        result = DSA.indexer_top_k_wrapper(
            input_values, seq_lens, top_k, next_n=next_n,
            return_val=return_val, stream=stream,
        )
    except (ValueError, NotImplementedError, RuntimeError) as e:
        pytest.skip(f"Unsupported testcase: {e}")
