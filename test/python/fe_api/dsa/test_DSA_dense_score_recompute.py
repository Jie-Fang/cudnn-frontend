...
def test_DSA_dense_score_recompute_wrapper(
    ...
    cfg = dsa_init(
        ...
        s_q_default=256, s_kv_default=1024,
    )
    ...
    try:
        if score_type == "indexer":
            result = DSA.dense_indexer_score_recompute_wrapper(
                q, k, aux,
                qhead_per_kv_head=qhead_per_kv_head, stream=stream,
            )
        else:
            softmax_scale = 1.0 / math.sqrt(head_dim)
            result = DSA.dense_attn_score_recompute_wrapper(
                q, k, aux, softmax_scale,
                qhead_per_kv_head=qhead_per_kv_head, stream=stream,
            )
    ...
    if not cfg["skip_ref"]:
        if score_type == "indexer":
            check_ref_dense_score_recompute(
                "indexer", q, k, aux, out, denom,
            )
        else:
            check_ref_dense_score_recompute(
                "attention", q, k, aux, out, denom,
