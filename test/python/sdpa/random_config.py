...
class RandomizationContext:
    ...
    def __call__(self, rng, rng_data_seed, rng_geom_seed=None):
        ...
        # TODO: Stats output (unlike score_max and score_sum_exp) doesn't seem to support anything other than BHSD
        # in the non-ragged case for some reason, so use a workaround for now. Investigate and remove this
        # workaround once fixed.
        if randoms_.is_ragged:
            randoms_.stride_stats_workaround = randoms_.stride_stats
        else:
            randoms_.stride_stats_workaround = get_strides_from_layout(randoms_.shape_stats, "bhsd")
