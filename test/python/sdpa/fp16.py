...
def create_forward_graph(cfg, tensors, cudnn_handle):
    ...
    if cfg.is_train:
        stats.set_uid(int(TensorUid.stats)).set_output(True).set_data_type(cudnn.data_type.FLOAT).set_dim(cfg.shape_stats).set_stride(cfg.stride_stats_workaround)
        if cfg.is_ragged:
...
def create_backward_graph(cfg, tensors, cudnn_handle, max_t_q, max_t_kv):
    ...
    stats = graph.tensor(uid=int(TensorUid.stats), dim=cfg.shape_stats, stride=cfg.stride_stats_workaround, data_type=cudnn.data_type.FLOAT)
