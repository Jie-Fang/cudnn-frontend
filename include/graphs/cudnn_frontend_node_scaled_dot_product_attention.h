...
namespace cudnn_frontend::graph {
    ...
    public:
        ...
        ScaledDotProductAttentionNode(std::string const& name, std::shared_ptr<Scaled_dot_product_attention> const options, int64_t const offset = 1)  : INode (name, offset), options(options) {
            P = std::make_shared<Tensor>("P"); // A dummy underlying tensor whose properties will be filled in infer_properties()
            ...
            auto bmm1_node = std::make_shared<MatMulNode>(bmm1_options->get_name(), bmm1_options, offset);
            ...
            bmm2_options->inputs.A = bmm1_options->outputs.C; // connect them bmms
            ...
            auto bmm2_node = std::make_shared<MatMulNode>(bmm2_options->get_name(), bmm2_options, offset + 100);
        ...
        error_t infer_properties() override final {
            ...
            P->set_uid(offset + 7)
             ...
             .set_dim({q_dim[0], q_dim[1], q_dim[2], k_dim[3]})
             .set_stride({1,1,1,1})
             .set_is_virtual(true);
