   ...
   public:
    ...
    validate_node() const override final {
        ...
        auto const& dropout_mask =
            attributes.inputs.find(SDPA_FP8_attributes::input_names::Dropout_mask);
        ...
        auto const& seq_len_q =
            attributes.inputs.find(SDPA_FP8_attributes::input_names::SEQ_LEN_Q);
        bool const has_seq_len_q = (seq_len_q != attributes.inputs.end()) && (seq_len_q->second != nullptr);

        auto const& seq_len_kv =
            attributes.inputs.find(SDPA_FP8_attributes::input_names::SEQ_LEN_KV);
    ...
    infer_properties_node() override final {
        ...
        if (attributes.inputs[SDPA_FP8_attributes::input_names::ragged_offset_QKV] != nullptr) {
            attributes.inputs[SDPA_FP8_attributes::input_names::Q]->set_ragged_offset(attributes.inputs[SDPA_FP8_attributes::input_names::ragged_offset_QKV]);
            attributes.inputs[SDPA_FP8_attributes::input_names::K]->set_ragged_offset(attributes.inputs[SDPA_FP8_attributes::input_names::ragged_offset_QKV]);
            attributes.inputs[SDPA_FP8_attributes::input_names::V]->set_ragged_offset(attributes.inputs[SDPA_FP8_attributes::input_names::ragged_offset_QKV]);
        }
        ...
        auto bmm1_attributes = Matmul_attributes().set_name("Q*K^T").set_m_override(seq_len_q).set_n_override(seq_len_kv);
        last_output =
            matmul(attributes.inputs[SDPA_FP8_attributes::input_names::Q],
                   last_output,
                   bmm1_attributes);
        ...
        if (attributes.inputs[SDPA_FP8_attributes::input_names::Attn_scale]) {
            ...
            last_output = pointwise(last_output, attributes.inputs[SDPA_FP8_attributes::input_names::Attn_scale], scale_attributes);
        ...
        last_output = pointwise(last_output, attributes.inputs[SDPA_FP8_attributes::input_names::descale_Q], descale_q_attributes);
        ...
        last_output = pointwise(last_output, attributes.inputs[SDPA_FP8_attributes::input_names::descale_K], descale_k_attributes);
        ...
        auto softmax_attributes =
            Softmax_attributes().set_name("softmax").has_M_Zinv(true).has_stats(false);
        // Special non-functional-style call. Needed because output already created and provided to user.
        softmax(last_output, softmax_attributes, softmax_output, attributes.outputs[SDPA_FP8_attributes::output_names::M], attributes.outputs[SDPA_FP8_attributes::output_names::Zinv]);
        ...
        bool dropout_present = (attributes.dropout_probability.has_value() && attributes.dropout_probability.value() != 0.0);
        ...
        if (dropout_present) {
            ...
            auto const& rng_output =
                rng(attributes.inputs[SDPA_FP8_attributes::input_names::Seed],
        ...
        last_output = pointwise(last_output, attributes.inputs[SDPA_FP8_attributes::input_names::scale_S], scale_s_attributes);
        last_output->set_name("S_s").set_data_type(DataType_t::FP8_E4M3);
        
        auto bmm2_attributes = Matmul_attributes().set_name("bmm2").set_m_override(seq_len_q).set_k_override(seq_len_kv);
        last_output =
            matmul(last_output, attributes.inputs[SDPA_FP8_attributes::input_names::V],
                   bmm2_attributes);
        ...
        ///////////////// TODO /////////////////////////////////
        last_output = pointwise(last_output, attributes.inputs[SDPA_FP8_attributes::input_names::scale_S], descale_s_attributes);
        // This above wrong! Should be attributes.inputs[SDPA_FP8_attributes::input_names::scale_dS]. Decide if dS should be allocated in extra workspace
        // or trouble the user to provide and maintain yet another unecessary detail
        ...
        last_output = pointwise(last_output, attributes.inputs[SDPA_FP8_attributes::input_names::descale_V], descale_v_attributes);
