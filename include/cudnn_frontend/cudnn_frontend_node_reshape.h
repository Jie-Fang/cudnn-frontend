...
public:
    ReshapeNode(Reshape_attributes&& options_, detail::Context const& context)  : INode (context), options(std::move(options_)) {}
    ...
    error_t validate_node() const override final {
        getLogger() << "[cudnn_frontend] INFO: " << "Validating ReshapeNode " << options.name << "..." << std::endl;
    ...
    error_t infer_properties_node() override final {
        getLogger() << "[cudnn_frontend] INFO: Inferrencing properties for reshape node " << options.name << "..." << std::endl;
    ...
    error_t createTensors() override final {
        getLogger() << "[cudnn_frontend] INFO: " << "Building Reshape tensors " << options.name << "..." << std::endl;
    ...
    error_t createOperations() override final {
        ...
        getLogger() << "[cudnn_frontend] INFO: " << "Building ReshapeNode operations " << options.name << "..." << std::endl;
