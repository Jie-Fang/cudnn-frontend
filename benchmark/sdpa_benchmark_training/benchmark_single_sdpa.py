...
parser.add_argument("--data_type", default="bfloat16", type=str, help="Data type to input to the layer. Can be bfloat16, float16, or fp8")
...
parser.add_argument("--sdpa_backend", default="pyt_cudnn", type=str, 
                    ...
                    choices=["pyt_native", "pyt_math", "pyt_cudnn", "pyt_efficient_attention", 
                             "pyt_flash_attention", "flash_attention", "flash_attention_3", "cudnn_fe"])
...
if args.sdpa_backend == 'cudnn_fe':
    ...
    if args.data_type == 'fp8':
        ...
        query = torch.randn(batch_size, num_q_heads, q_seqlen, head_dim, dtype=target_dtype, device=device)
        key = torch.randn(batch_size, num_kv_heads, kv_seqlen, head_dim, dtype=target_dtype, device=device)
        value = torch.randn(batch_size, num_kv_heads, kv_seqlen, head_dim, dtype=target_dtype, device=device)
        output = torch.empty(batch_size, num_q_heads, q_seqlen, head_dim, dtype=target_dtype, device=device)
    ...
    if args.fwd_bwd:
        graph_bwd = cudnn.pygraph(
            io_data_type=cudnn.data_type.FP8_E4M3 if args.data_type == 'fp8' else cudnn.data_type.HALF,
            intermediate_data_type=cudnn.data_type.FLOAT,
            compute_data_type=cudnn.data_type.FLOAT,
        )
...
for i in range(num_iters):
    if args.data_type == "fp8" and args.sdpa_backend == 'cudnn_fe':
        ...
        query = torch.randn(batch_size, num_q_heads, q_seqlen, head_dim, dtype=torch.bfloat16, device=device, requires_grad=True).to(torch.float8_e4m3fn)
        key = torch.randn(batch_size, num_kv_heads, kv_seqlen, head_dim, dtype=torch.bfloat16, device=device, requires_grad=True).to(torch.float8_e4m3fn)
        value = torch.randn(batch_size, num_kv_heads, kv_seqlen, head_dim, dtype=torch.bfloat16, device=device, requires_grad=True).to(torch.float8_e4m3fn)
    else:
        query = torch.randn(batch_size, num_q_heads, q_seqlen, head_dim, dtype=target_dtype, device=device, requires_grad=True)
        key = torch.randn(batch_size, num_kv_heads, kv_seqlen, head_dim, dtype=target_dtype, device=device, requires_grad=True)
        value = torch.randn(batch_size, num_kv_heads, kv_seqlen, head_dim, dtype=target_dtype, device=device, requires_grad=True)

    query, key, value = preprocess_qkv(query, key, value, args.sdpa_backend)
    
    if args.sdpa_backend == 'cudnn_fe':
        output = torch.empty(batch_size, num_q_heads, q_seqlen, head_dim, dtype=torch.uint8 if args.data_type == "fp8" else target_dtype, device=device)
        dQuery = torch.empty_like(query)
        dKey = torch.empty_like(key)
        dValue = torch.empty_like(value)
        dOutput = torch.empty_like(output)
