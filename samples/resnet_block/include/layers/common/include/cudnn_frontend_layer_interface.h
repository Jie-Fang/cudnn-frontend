...
/**
 * @brief A Problem Descriptor builder class to configure and and build problme descriptors such as convolution descriptors, pointwise descriptors, etc. This class is used within the building blocks to configure and build problem descriptors.
 * 
 */
class ProblemDescBuilder {
    public:
         ...
         */
        void configureMathPrec(const cudnnDataType_t &mathPrec) {
            mathPrec_ = mathPrec;
        };
         ...
         */
        void configurePointWiseMode(const cudnnPointwiseMode_t &mode) {
            pwMode_ = mode;
        };

        /**
         * @brief Builds the matmul descriptor. All is needed is the math precision configured. If NV_CUDNN_DISABLE_EXCEPTION is not defined, it throws an exception if the problem descriptor failed to build. Otherwise, it returns the descriptor with a bad status. If the problem descriptor successfully builds, it returns normally.
         * 
         * @return MatMulDesc A built matmul descriptor.
         */
        MatMulDesc buildMatmul() {
            ...
            return MatMulDescBuilder().setMathPrecision(mathPrec_).build();
                ...
                std::string error_message = std::string("Error in creating matmul problem descriptor. Error message: ") + e.what();
        ...
        /**
         * @brief Builds a convolution descriptor. If NV_CUDNN_DISABLE_EXCEPTION is not defined, it throws an exception if the problem descriptor failed to build. Otherwise, it returns the descriptor with a bad status. If the problem descriptor successfully builds, it returns normally.
         * 
         * @param convParams A struct containing the convolution parameters.
         * @return ConvDesc_v8 Returns a built convolution descriptor.
         */
        ConvDesc_v8 buildConv(const ConvParams &convParams) {
            #ifndef NV_CUDNN_DISABLE_EXCEPTION
            try {
            #endif
            return ConvDescBuilder_v8()
                ...
                std::string error_message = std::string("Error in creating convolution problem descriptor. Error message: ") + e.what();
                throw cudnnException(error_message.c_str(), e.getCudnnStatus());
            }
            #endif
        };
        ...
        /**
         * @brief Builds a resampling descriptor. If NV_CUDNN_DISABLE_EXCEPTION is not defined, it throws an exception if the problem descriptor failed to build. Otherwise, it returns the descriptor with a bad status. If the problem descriptor successfully builds, it returns normally.
         * 
         ...
         */
        ResampleDesc_v8 buildResample(PoolingParams &poolingParams) {
        ...
        /**
         * @brief Builds a pointwise descriptor. If NV_CUDNN_DISABLE_EXCEPTION is not defined, it throws an exception if the problem descriptor failed to build. Otherwise, it returns the descriptor with a bad status. If the problem descriptor successfully builds, it returns normally.
         * 
         * @return PointWiseDesc A built pointwise descriptor.
         */
        PointWiseDesc buildPointwiseDesc() {
            #ifndef NV_CUDNN_DISABLE_EXCEPTION
            try {
            #endif

            return PointWiseDescBuilder().setMode(pwMode_)
                                                .setMathPrecision(mathPrec_)
                                                .build();
            #ifndef NV_CUDNN_DISABLE_EXCEPTION
            } catch (cudnn_frontend::cudnnException & e) {
                std::string error_message = std::string("Error in creating bias problem descriptor. Error message: ") + e.what();
                throw cudnnException(error_message.c_str(), e.getCudnnStatus());
            }
            #endif
        };
...
/**
 * @brief A parent class for building blocks. A block is a collection of operations/operation graphs that can be executed with a single function call. The IBlock class contain methods to build the operation graph,
 * create execution plans, get workspace size, and execute the block. It also contains maps to store
 * operation graphs, tensors, variant packs, etc.
 * 
 */
class IBlock {
...
protected:
    ...
    calculatePlanExecutionWorkspaceSize() {
        ...
        getLogger() << "[cudnn_frontend] " << "INFO: Plan execution Workspace size is " << max_workspace_size << std::endl;
    ...
    getPlanFromHeuristics(const std::string& graph_name, OperationGraph_v8 &opGraph, cudnnHandle_t &handle) {
        ...
        getLogger() << "[cudnn_frontend] " << "Filter config list has " << filtered_configs.size() << " configurations " << std::endl;
        ...
        for (size_t i = 0; i < filtered_configs.size(); i++) {
            ...
            if (i == filtered_configs.size() - 1 && plan.get_status() != CUDNN_STATUS_SUCCESS) {
                logErrorMessage(CUDNN_STATUS_EXECUTION_FAILED, blockName, "execution plan descriptor", graph_name, error_message, plan.get_error());
                return plan;
            ...
            return plan;
                ...
                if (i == filtered_configs.size() - 1) {
                    logErrorMessage(e.getCudnnStatus(), blockName, "execution plan descriptor", graph_name, error_message, e.what());
                    throw cudnnException(error_message.c_str(), e.getCudnnStatus());
                }
                continue;
            }
...
public:
    ...
    /**
     * @brief Virtual createVariantPacks method. Create the necessary variant packs for the block pass. Done internally for the user. All the user needs to pass in is A ptr to a ResidualBlockDevPtrStore object which contains the necessary pointers to the device memory. See resnet_sample.cpp and resnet_test_list.cpp for examples.
     * 
     * @param devPtrStore A ptr to A ptr to a ResidualBlockDevPtrStore object which contains the necessary pointers to the device memory (see cudnn_frontend_residual_block_dev_ptr_store.h for details).
     * @return cudnnStatus_t Returns CUDNN_STATUS_SUCCESS if all tensors were successfully created, otherwise, if NV_CUDNN_DISABLE_EXCEPTION is not defined, it returns a bad status with an error message. Otherwise, it throws an error for which tensor failed to build. Use getErrorMessage() to see error message.
     */
    virtual
    cudnnStatus_t createVariantPacks(ResidualBlockDevPtrStore* devPtrStore) {
        (void) devPtrStore;
        return CUDNN_STATUS_SUCCESS;
    }

    // /**
    //  * @brief Virtual method. Overload for the classifier block. Create the necessary variant packs for the block pass. Done internally for the user. All the user needs to pass in is A  ptr to a ClassifierBlockDevPtrStore object which contains the necessary pointers to the device memory. See resnet_sample.cpp and resnet_test_list.cpp for examples.
    //  * 
    //  * @param devPtrStore A ptr to A  ptr to a ClassifierBlockDevPtrStore object which contains the necessary pointers to the device memory (see cudnn_frontend_classifier_block_dev_ptr_store.h for details).
    //  * @return cudnnStatus_t Returns CUDNN_STATUS_SUCCESS if all tensors were successfully created, otherwise, if NV_CUDNN_DISABLE_EXCEPTION is not defined, it returns a bad status with an error message. Otherwise, it throws an error for which tensor failed to build. Use getErrorMessage() to see error message.
    //  */
    // virtual
    ...
    // /**
    //  * @brief Virtual method. Overload for the GPT block. Create the necessary variant packs for the block pass. Done internally for the user. All the user needs to pass in is a GPTDevPtrStore object which contains the necessary pointers to the device memory. See resnet_sample.cpp and resnet_test_list.cpp for examples.
    //  * 
    //  * @param devPtrStore A ptr to a GPTDevPtrStore object which contains the necessary pointers to the device memory (see cudnn_frontend_gpt_dev_ptr_store.h for details).
    //  * @return cudnnStatus_t Returns CUDNN_STATUS_SUCCESS if all tensors were successfully created, otherwise, if NV_CUDNN_DISABLE_EXCEPTION is not defined, it returns a bad status with an error message. Otherwise, it throws an error for which tensor failed to build. Use getErrorMessage() to see error message.
    //  */
    // virtual
     ...
     */
    const cudnnStatus_t& getStatus() const {
        return status_;
    }
     ...
     * 
     * @return cudnnStatus_t A cudnnStatus_t value indicating whether or not the allocation was successful. If it built, it returns CUDNN_STATUS_SUCCESS. Otherwise, it may return CUDNN_STATUS_BAD_PARAM, e.getCudnnStatus(), etc.
     */
    cudnnStatus_t setWorkspace(void *ptr) {
     ...
     */
    void addSubOpGraph(const std::string &subgraph_name, SubGraph const &sub_graph) {
     ...
     * @param handle A cudnn handle
     */
    cudnnStatus_t build(cudnnHandle_t handle) {
        ...
        for (auto &sub_graph : sub_graphs) {
            ...
            if (opGraphStatus != CUDNN_STATUS_SUCCESS) {
                getLogger() << "[cudnn_frontend] " << "[ERROR] building operation graph for graph " << sub_graph.first << ". Error message: " << error_message << std::endl;
                return opGraphStatus;
            }

            auto plan = getPlanFromHeuristics(sub_graph.first, op_graphs.at(sub_graph.first), handle);
            if (plan.get_status() != CUDNN_STATUS_SUCCESS) {
                getLogger() << "[cudnn_frontend] " << "[ERROR] building execution plan for graph " << sub_graph.first << ". Error message: " << error_message << std::endl;
     ...
     */
    void updateVariantPackforPlan(const std::string& name, VariantPack_v8& variant_pack) {
            auto it = variant_packs.find(name);
            if (it != variant_packs.end()) {
                it->second = std::move(variant_pack);
            } else {
                getLogger() << "[cudnn_frontend] " << "INFO: Could not find plan " << name << " in variant pack map. Creating new variant pack for plan." << std::endl;
     ...
     * @return cudnnStatus_t A cudnnStatus_t value indicating whether or not the executions plan executed successfully. If so, it returns a CUDNN_STATUS_SUCCESS. Otherwise, if NV_CUDNN_DISABLE_EXCEPTION is enabled, it returns a bad status. If not, it throws an exception. 
     */
    virtual
    cudnnStatus_t execute(cudnnHandle_t& handle) {
     ...
     * @return cudnnStatus_t A cudnnStatus_t value indicating whether or not the executions plan executed successfully. If so, it returns a CUDNN_STATUS_SUCCESS. Otherwise, if NV_CUDNN_DISABLE_EXCEPTION is enabled, it returns a bad status. If not, it throws an exception. 
     */
    cudnnStatus_t executePlan(cudnnHandle_t& handle, const std::string &plan) {
    ...
    cudnnStatus_t executePlanAndLogTime(cudnnHandle_t handle, ExecutionPlan_v8 &plan, const VariantPack_v8 &variantPack, const std::string &planName) {
        ...
        getLogger() << "[cudnn_frontend] Plan " << planName << " took " << std::setw(10) << final_time_ms << " " << "milliseconds" << std::endl;
        getLogger() << "[cudnn_frontend] Plan tag " << plan.getTag() << std::endl;
        return CUDNN_STATUS_SUCCESS;
    }

    cudnnStatus_t executePlanOnce(cudnnHandle_t handle, ExecutionPlan_v8 &plan, const VariantPack_v8 &variantPack, const std::string &planName) {
        (void) planName;
        cudnnBackendExecute(handle, plan.get_raw_desc(), variantPack.get_raw_desc());
        getLogger() << "[cudnn_frontend] Plan tag " << plan.getTag() << std::endl;
