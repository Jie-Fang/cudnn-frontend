...
make_convolution_params(convolution_params::uids_t const uid_offset, int64_t const N, int64_t const C, int64_t const H, int64_t const W, int64_t const K, int64_t const R, int64_t const S, int64_t const pad, int64_t const stride) {
    return {
            /* dim count  */       4,
            /* input dim  */       {N, C, H, W},
            /* weight dim */       {K, C, R, S},
            /* output dim */       {N, K, static_cast<int64_t>(std::ceil((H + (2 * pad) - R + 1) / stride)), 
            ...
            /* uids */            {uid_offset + convolution_params::UIDs::INPUT_UID, 
...
make_bn_fusion_params(bn_fusion_params::uids_t uid_offset, int64_t const N, int64_t const C, int64_t const H, int64_t const W) {
    return {
        /* dim count  */       4,
        /* input dim  */       {N, C, H, W},
        /* channel dim  */     {1, C, 1, 1},
        ...
        /* uids */             {uid_offset + bn_fusion_params::UIDs::INPUT_UID, 
...
make_pointwise_parameters(pointwise_parameters::uids_t uid_offset, int64_t const N, int64_t const C, int64_t const H, int64_t const W) {
    return {
        /* dim count  */       4,
        /* input dim  */       {N, C, H, W},
        /* weight dim  */      {-1, -1, -1, -1},
 ...
 * @return int64_t 
 */
static inline int64_t calculatePoolingOutputSize(const int64_t &input_size, const int64_t &window_size, const int64_t &stride, const int64_t &padding) {
    return (input_size + 2 * padding - window_size) / stride + 1;
}

// Helper functions to calculate device ptr sizes.
static inline int64_t calculateSize(const int64_t *dims, int numDims) {
    int64_t size = 1;
...
static inline bool isDimensionMatch(const int64_t *A, const int64_t *B, int numDims) {
    for (int i = 0; i < numDims; i++) {
...
/**
 * @brief C++ implementation of Python instanceof method. Checks to see whether or not an object is instance of some class.
 * 
...
/**
 * @brief Helper error logging function Uses a generic type T for the block calling the error, that way the user knows what block the error is happening. It sets a passed in reference error_message to be used later by the user or internally.
 * 
 * @param blockName The name of the block as a string.
 * @param descriptorType A string denoting a descriptor type. This allows for more specific error messages. Descriptor type examples include "tensor", "problem descriptor", "operation graph", "execution plan" etc. 
 ...
 * @param extraMessage Any extra message to be appeneded to the error message. Usually this is an exception message, or CUDNN descriptor get_error() string. Defaults to empty string.
 * @return bool Returns true if there was an error. The error message gets set. Returns false if there is no error (e.g. status == CUDNN_STATUS_SUCCESS)
 */
static inline void logErrorMessage(cudnnStatus_t status, const std::string& blockName, const std::string& descriptorType, const std::string &descriptorName, std::string& errorMessage, const std::string &extraMessage="") {
    if (descriptorType == "execution plan") {
        errorMessage = "[ERROR]: CUDNN Error in exeucting plan with name: " + descriptorName + "! This error happens in the " + blockName + "! Double check your parameters/device pointers. CUDNN error string: " + cudnnGetErrorString(status) + " " + extraMessage;

    } else if (descriptorType == "params builder") {
        errorMessage = "[ERROR]: Error in creating : " + descriptorType  + " with name: " + descriptorName + "! This error happens in the " + blockName + "! Double check your parameters and read error string to see what's wrong. Error string: " + cudnnGetErrorString(status) + " " + extraMessage;

    } else if (descriptorType == "device pointer store") {
        errorMessage = "[ERROR]: Error in creating : " + descriptorType  + " with name: " + descriptorName + "! This error happens in the " + blockName + "! Double check that your device pointers are set correctly and all necessary device pointers are set. Read error string to see what's wrong. Error string: " + cudnnGetErrorString(status) + " " + extraMessage;

    } else {
        errorMessage = "[ERROR]: CUDNN Error in creating descriptor type: " + descriptorType  + " with name: " + descriptorName + "! This error happens in the " + blockName + "! Double check your parameters/device pointers. CUDNN error string: " + cudnnGetErrorString(status) + " " + extraMessage;
    }
};

/**
 * @brief Helper error logging function to check for errors based on a cudnnStatus_t. Used internally when NV_CUDNN_DISABLE_EXCEPTION is enabled since exceptions won't be thrown. Instead, this function is used to log helpeful error messages. Uses a generic type T for the block calling the error, that way the user knows what block the error is happening. It sets a passed in reference error_message to be used later by the user or internally.
 * 
 ...
 * @return bool Returns true if there was an error. The error message gets set. Returns false if there is no error (e.g. status == CUDNN_STATUS_SUCCESS)
 */
static inline bool checkErrorStatusAndLog(cudnnStatus_t status, const std::string& blockName, const std::string& descriptorType, const std::string &descriptorName, std::string& errorMessage, const std::string &extraMessage="") {
